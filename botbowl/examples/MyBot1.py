...
class MyBot1(botbowl.ProcBot):
    ...
    #esta funcion está distinta en el ejemplo de scripted_bot_example
    ...
    def turn(self, game):
        ...
        #No entiendo por qué se hace esto antes que "make_plan"
    ...
    #
    #     # 1. Stand up players
    #     self.stand_up_players()
    #
    #     # 2. Move the ball carrier
    #     self.move_ball_carrier(ball_carrier)
    #
    #     # 3. Make safe blocks
    #     self.make_self_blocks()
    #
    #     # 4. Pikcup the ball
    #     self.pick_up_ball()
    #
    #     # 5. Move receivers into scoring range
    #     self.receivers_into_scoring_zone(ball_carrier, open_players)
    #
    #     # 6. Make a blitz move
    #     self.make_blitz_move(open_players)
    #
    #     # 7. Make cage around ball carrier
    #     self.surround_ball_carrier(ball_carrier, open_players)
    #
    #     # 8. Move non-marked players to assist
    #     self.move_non_marked_players(open_players)
    #
    #     # 9. Move towards the ball
    #     self.move_towards_the_ball(ball_carrier, open_players)
    #
    #     # 10.Make risky blocks
    #     self.risky_blocks()
    #
    ...
    #                 return
    #
    # def move_ball_carrier(self, ball_carrier):
    #     if (ball_carrier is not None and ball_carrier.team == self.my_team and not ball_carrier.state.used):
    #         # 2.1 Can ball carrier score with high probability
    #         score_path = pf.get_safest_path_to_endzone(game, ball_carrier, allow_team_reroll=True)
    #
    #         #entiendo que el 0.75 depende del programador, como de exigente quiera ser
    #         #en el ejemplo usa 0.7
    #         if score_path is not None and score_path.prob > 0.75:
    #             self.actions.append(Action(ActionType.START_MOVE, player=ball_carrier))
    #             self.actions.extend(path_to_move_actions(game, ball_carrier, score_path))
    #             return
    #
    ...
    #                 return
    #
    # def make_self_blocks(self):
    #     attacker, defender, p_self_up, p_opp_down, block_p_fumble_self, block_p_fumble_opp = self._get_safest_block(game)   #no entiendo esta linea
    #     #ESTO NO TIENE EN CUENTA LOS REROLLS DE LOS DADOS
    #     if attacker is not None and p_self_up > 0.94 and block_p_fumble_self == 0:
    #         self.actions.append(Action(ActionType.START_BLOCK, player=attacker))
    #         self.actions.append(Action(ActionType.BLOCK, position=defender.position))
    #         return
    ...
    #             return
    #
    # def receivers_into_scoring_zone(self, ball_carrier, open_players):
    #
    #     for player in open_players:
    #         if player.has_skill(Skill.CATCH) and player != ball_carrier:
    #             if game.get_distance_to_endzone(player) > player.num_moves_left():
    ...
    #                 return
    #
    # def make_blitz_move(self, open_players):
    #     if game.is_blitz_available():
    #
    #         best_blitz_attacker = None
    #         best_blitz_defender = None
    ...
    #             return
    #
    # def surround_ball_carrier(self, ball_carrier, open_players):
    #     #como rodear al jugador (TL, TR, BL, BR)
    #     cage_positions = [
    #         Square(game.get_ball_position().x - 1, game.get_ball_position().y - 1),
    #         Square(game.get_ball_position().x + 1, game.get_ball_position().y - 1),
    #         Square(game.get_ball_position().x - 1, game.get_ball_position().y + 1),
    ...
    #                         return
    #
    # def move_non_marked_players(self, open_players):
    #     # Scan for assist positions
    #     assist_positions = set()
    #     for player in game.get_opp_team(self.my_team).players:
    #         if player.position is None or not player.state.up:
    ...
    #             return
    #
    # def move_towards_the_ball(self, ball_carrier, open_players):
    #     for player in open_players:
    #         if player == ball_carrier or game.num_tackle_zones_in(player) > 0:
    #             continue
    #
...
botbowl.register_bot('my-random-bot', MyBot1)   #esto creo que hay que cambiar el nombre sin mas
